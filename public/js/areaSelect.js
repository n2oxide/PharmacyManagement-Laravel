var addressSelect = function(_provinceSelect,_citySelect,_areaSelect)
{
	var provinceSelect = document.getElementById(_provinceSelect);
	var citySelect = document.getElementById(_citySelect);
	var areaSelect = document.getElementById(_areaSelect);
	
	var selectedProvine;
	var selectedCity;
	var selectedArea;
	function addOption(selectDOM,addOptions)
	{
		...
		{
			var option = document.createElement("option");
			option.innerHTML = typeof(addOptions[i].name)!="undefined"?addOptions[i].name:addOptions[i];
			option.value = typeof(addOptions[i].name)!="undefined"?addOptions[i].name:addOptions[i];
			selectDOM.options.add(option);
		}
	}
	
	function getIndex(selectDom,valueStr)
	{
		for(var i = 0; i<selectDom.options.length; i++)
		{
			if(selectDom.options[i].value==valueStr)
				return i-1;
		}
	}
	...
	{
		addOption(provinceSelect,provinceList);
	}
	
	initProvince();
	
	$("#"+_provinceSelect).change(function(e){
		citySelect.options.length=0;
		areaSelect.options.length=0;
		var provinceI = getIndex(e.target,e.target.value);
		selectedProvine = provinceList[provinceI];
		addOption(citySelect,selectedProvine.cityList);
	});
	
	$("#"+_citySelect).change(function(e){
		areaSelect.options.length=0;
		var cityI = getIndex(e.target,e.target.value)
		selectedCity = selectedProvine.cityList[cityI];
		addOption(areaSelect,selectedCity.areaList);
	});
}
